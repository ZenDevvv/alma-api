...
describe("Organization Controller", () => {
	...
	const mockOrganization = {
		...
		name: "User Registration Organization",
		description: "Organization for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};
	...
	const mockOrganizations = [
		...
		},
		{
			...
			name: "SMS Notification Organization",
			description: "Organization for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			name: "Email Marketing Organization",
			description: "Organization for email marketing campaigns",
			type: "email",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "Organization without type",
			type: null,
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			organization: {
				findMany: async (_params: Prisma.OrganizationFindManyArgs) => {
					// Return multiple organizations for grouping tests
					if (req.query?.groupBy) {
						return mockOrganizations;
					}
					return [mockOrganization];
				},
				count: async (_params: Prisma.OrganizationCountArgs) => {
					// Return count based on whether grouping is requested
					if (req.query?.groupBy) {
						return mockOrganizations.length;
					}
					return 1;
				},
				...
				delete: async (params: Prisma.OrganizationDeleteArgs) => ({
					...mockOrganization,
					id: params.where.id,
				}),
			},
	...
	describe(".getAll()", () => {
		...
		it("should group organizations by type field", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should group organizations by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.grouped).to.have.property("User Registration Organization");
			expect(sentData.data.grouped).to.have.property("SMS Notification Organization");
		});

		it("should handle organizations with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type" };
			await organizationController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
	...
	describe(".getById()", () => {
		it("should return a organization", async function () {
			this.timeout(TEST_TIMEOUT);
	...
	describe(".create()", () => {
		it("should create a new organization", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Contact Form Organization",
				description: "Organization for contact forms with validation",
			};
			req.body = createData;
		...
		it("should create a new organization with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email Organization",
				description: "Organization for email notifications",
				type: "email",
			};
			req.body = createData;
			await organizationController.create(req as Request, res, next);
		...
		it("should create a new organization without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic Organization",
				description: "Organization without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				...
				description: "Organization from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				...
				description: "Organization from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				...
				description: "Organization that will cause Prisma error",
			};
			req.body = createData;

			// Mock Prisma to throw an error
			prisma.organization.create = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Organization",
				description: "Organization that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update organization details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Organization",
				description: "Updated organization with additional validation and styling options",
			};
			req.params = { id: mockOrganization.id };
			req.body = updateData;
		...
		it("should update organization type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple organization fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Email Organization",
		...
		it("should handle form data (multipart/form-data)", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Form Updated Organization",
		...
		it("should handle non-existent organization update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Organization",
			};
	...
	describe(".remove()", () => {
		it("should delete a organization", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockOrganization.id };
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long organization name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
		...
		it("should handle special characters in organization data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
		...
		it("should handle concurrent requests", async function () {
			...
			const createData = {
				...
				description: "Organization created concurrently",
			};
			req.body = createData;
			...
			const promises = Array(5)
				.fill(null)
				.map(() => organizationController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "Organization 1", type: "email", category: "marketing" },
		{ id: 2, name: "Organization 2", type: "sms", category: "notification" },
		{ id: 3, name: "Organization 3", type: "email", category: "marketing" },
		{ id: 4, name: "Organization 4", type: null, category: "general" },
		{ id: 5, name: "Organization 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "Organization 1", type: "email" },
				{ id: 2, name: "Organization 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("Organization 1");
			expect(result).to.have.property("Organization 2");
			expect(result).to.have.property("Organization 3");
			expect(result).to.have.property("Organization 4");
			expect(result).to.have.property("Organization 5");
			expect(result["Organization 1"]).to.have.length(1);
		});
	});
});
