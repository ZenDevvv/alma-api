import { controller } from "../app/stock/stock.controller";
...
describe("Stock Controller", () => {
	...
	const mockStock = {
		...
		name: "User Registration Stock",
		description: "Stock for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};
	...
	const mockStocks = [
		...
		},
		{
			...
			name: "SMS Notification Stock",
			description: "Stock for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			name: "Email Marketing Stock",
			description: "Stock for email marketing campaigns",
			type: "email",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			name: "Generic Stock",
			description: "Stock without type",
			type: null,
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			stock: {
				findMany: async (_params: Prisma.StockFindManyArgs) => {
					// Return multiple stocks for grouping tests
					if (req.query?.groupBy) {
						return mockStocks;
					}
					return [mockStock];
				},
				count: async (_params: Prisma.StockCountArgs) => {
				...
				findFirst: async (params: Prisma.StockFindFirstArgs) =>
					params.where?.id === mockStock.id ? mockStock : null,
				findUnique: async (params: Prisma.StockFindUniqueArgs) =>
					params.where?.id === mockStock.id ? mockStock : null,
				create: async (params: Prisma.StockCreateArgs) => ({
					...mockStock,
					...params.data,
				}),
				update: async (params: Prisma.StockUpdateArgs) => ({
					...mockStock,
					...params.data,
				}),
				delete: async (params: Prisma.StockDeleteArgs) => ({
					...mockStock,
					id: params.where.id,
				}),
			},
		...
		stockController = controller(prisma as PrismaClient);
		sentData = undefined;
		statusCode = 200;
		req = {
			query: {},
	...
	describe(".getAll()", () => {
		it("should return paginated stocks", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { page: "1", limit: "10" };
			await stockController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
		});

		it("should group stocks by type field", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should group stocks by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.grouped).to.have.property("User Registration Stock");
			expect(sentData.data.grouped).to.have.property("SMS Notification Stock");
		});

		it("should handle stocks with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type" };
			await stockController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
	...
	describe(".getById()", () => {
		it("should return a stock", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockStock.id };
			await stockController.getById(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.deep.include({ id: mockStock.id });
		});
		...
		it("should handle non-existent stock", async function () {
			this.timeout(TEST_TIMEOUT);
	...
	describe(".create()", () => {
		it("should create a new stock", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Contact Form Stock",
				description: "Stock for contact forms with validation",
			};
			req.body = createData;
			await stockController.create(req as Request, res, next);
		...
		it("should create a new stock with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email Stock",
				description: "Stock for email notifications",
				type: "email",
			};
			req.body = createData;
			await stockController.create(req as Request, res, next);
		...
		it("should create a new stock without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic Stock",
				description: "Stock without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				name: "Form Stock",
				description: "Stock from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				name: "URL Stock",
				description: "Stock from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle validation errors", async function () {
			...
			const createData = {
				name: "",
				description: "Stock with empty name",
			};
			req.body = createData;
			await stockController.create(req as Request, res, next);
			expect(statusCode).to.equal(400);
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				name: "Test Stock",
				description: "Stock that will cause Prisma error",
			};
			req.body = createData;

			// Mock Prisma to throw an error
			prisma.stock.create = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Stock",
				description: "Stock that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update stock details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Stock",
				description: "Updated stock with additional validation and styling options",
			};
			req.params = { id: mockStock.id };
			req.body = updateData;
			await stockController.update(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.have.property("id");
		...
		it("should update stock type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple stock fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Email Stock",
		...
		it("should handle non-existent stock update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Stock",
			};
	...
	describe(".remove()", () => {
		it("should delete a stock", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockStock.id };
			await stockController.remove(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle non-existent stock deletion", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: "507f1f77bcf86cd799439099" };
			await stockController.remove(req as Request, res, next);
			expect(statusCode).to.equal(404);
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long stock name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "A".repeat(1000), // Very long name
				description: "Stock with very long name",
			};
			req.body = createData;
			await stockController.create(req as Request, res, next);
			expect(statusCode).to.equal(201);
		...
		it("should handle special characters in stock data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Stock with special chars: !@#$%^&*()",
		...
		it("should handle concurrent requests", async function () {
			...
			const createData = {
				...
				description: "Stock created concurrently",
			};
			req.body = createData;
			...
			const promises = Array(5)
				.fill(null)
				.map(() => stockController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "Stock 1", type: "email", category: "marketing" },
		{ id: 2, name: "Stock 2", type: "sms", category: "notification" },
		{ id: 3, name: "Stock 3", type: "email", category: "marketing" },
		{ id: 4, name: "Stock 4", type: null, category: "general" },
		{ id: 5, name: "Stock 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "Stock 1", type: "email" },
				{ id: 2, name: "Stock 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("Stock 1");
			expect(result).to.have.property("Stock 2");
			expect(result).to.have.property("Stock 3");
			expect(result).to.have.property("Stock 4");
			expect(result).to.have.property("Stock 5");
			expect(result["Stock 1"]).to.have.length(1);
		});
	});
});
