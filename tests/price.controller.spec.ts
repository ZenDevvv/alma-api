import { controller } from "../app/price/price.controller";
...
describe("Price Controller", () => {
	...
	const mockPrice = {
		...
		name: "User Registration Price",
		description: "Price for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};
	...
	const mockPrices = [
		...
		},
		{
			...
			description: "Price for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "Price for email marketing campaigns",
			type: "email",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			name: "Generic Price",
			description: "Price without type",
			type: null,
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			price: {
				findMany: async (_params: Prisma.PriceFindManyArgs) => {
					// Return multiple prices for grouping tests
					if (req.query?.groupBy) {
						return mockPrices;
					}
					return [mockPrice];
				},
				count: async (_params: Prisma.PriceCountArgs) => {
					// Return count based on whether grouping is requested
					if (req.query?.groupBy) {
						return mockPrices.length;
					}
					return 1;
				},
				findFirst: async (params: Prisma.PriceFindFirstArgs) =>
					params.where?.id === mockPrice.id ? mockPrice : null,
				findUnique: async (params: Prisma.PriceFindUniqueArgs) =>
					params.where?.id === mockPrice.id ? mockPrice : null,
				create: async (params: Prisma.PriceCreateArgs) => ({
					...mockPrice,
					...params.data,
				}),
				update: async (params: Prisma.PriceUpdateArgs) => ({
					...mockPrice,
					...params.data,
				}),
				delete: async (params: Prisma.PriceDeleteArgs) => ({
					...mockPrice,
					id: params.where.id,
				}),
			},
		...
		priceController = controller(prisma as PrismaClient);
		sentData = undefined;
		statusCode = 200;
		req = {
			query: {},
	...
	describe(".getAll()", () => {
		it("should return paginated prices", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { page: "1", limit: "10" };
			await priceController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
		});

		it("should group prices by type field", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should group prices by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.grouped).to.have.property("User Registration Price");
			expect(sentData.data.grouped).to.have.property("SMS Notification Price");
		});

		it("should handle prices with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type" };
			await priceController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
	...
	describe(".getById()", () => {
		it("should return a price", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockPrice.id };
			await priceController.getById(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.deep.include({ id: mockPrice.id });
		});
		...
		it("should handle non-existent price", async function () {
			this.timeout(TEST_TIMEOUT);
	...
	describe(".create()", () => {
		it("should create a new price", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Contact Form Price",
				description: "Price for contact forms with validation",
			};
			req.body = createData;
			await priceController.create(req as Request, res, next);
		...
		it("should create a new price with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email Price",
				description: "Price for email notifications",
				type: "email",
			};
			req.body = createData;
			await priceController.create(req as Request, res, next);
		...
		it("should create a new price without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic Price",
				description: "Price without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				name: "Form Price",
				description: "Price from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				name: "URL Price",
				description: "Price from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle validation errors", async function () {
			...
			const createData = {
				name: "",
				description: "Price with empty name",
			};
			req.body = createData;
			await priceController.create(req as Request, res, next);
			expect(statusCode).to.equal(400);
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				name: "Test Price",
				description: "Price that will cause Prisma error",
			};
			req.body = createData;

			// Mock Prisma to throw an error
			prisma.price.create = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Price",
				description: "Price that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update price details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Price",
				description: "Updated price with additional validation and styling options",
			};
			req.params = { id: mockPrice.id };
			req.body = updateData;
			await priceController.update(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.have.property("id");
		...
		it("should update price type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple price fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Email Price",
		...
		it("should handle non-existent price update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Price",
			};
	...
	describe(".remove()", () => {
		it("should delete a price", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockPrice.id };
			await priceController.remove(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle non-existent price deletion", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: "507f1f77bcf86cd799439099" };
			await priceController.remove(req as Request, res, next);
			expect(statusCode).to.equal(404);
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long price name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "A".repeat(1000), // Very long name
				description: "Price with very long name",
			};
			req.body = createData;
			await priceController.create(req as Request, res, next);
			expect(statusCode).to.equal(201);
		...
		it("should handle special characters in price data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
		...
		it("should handle concurrent requests", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Concurrent Price",
				description: "Price created concurrently",
			};
			req.body = createData;
			...
			const promises = Array(5)
				.fill(null)
				.map(() => priceController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "Price 1", type: "email", category: "marketing" },
		{ id: 2, name: "Price 2", type: "sms", category: "notification" },
		{ id: 3, name: "Price 3", type: "email", category: "marketing" },
		{ id: 4, name: "Price 4", type: null, category: "general" },
		{ id: 5, name: "Price 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "Price 1", type: "email" },
				{ id: 2, name: "Price 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("Price 1");
			expect(result).to.have.property("Price 2");
			expect(result).to.have.property("Price 3");
			expect(result).to.have.property("Price 4");
			expect(result).to.have.property("Price 5");
			expect(result["Price 1"]).to.have.length(1);
		});
	});
});
