import { controller } from "../app/patient/patient.controller";
...
describe("Patient Controller", () => {
	...
	const mockPatient = {
		...
		description: "Patient for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};
	...
	const mockPatients = [
		...
		},
		{
			...
			description: "Patient for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "Patient for email marketing campaigns",
			type: "email",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "Patient without type",
			type: null,
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			patient: {
				findMany: async (_params: Prisma.PatientFindManyArgs) => {
					// Return multiple patients for grouping tests
					if (req.query?.groupBy) {
						return mockPatients;
					}
					return [mockPatient];
				},
				count: async (_params: Prisma.PatientCountArgs) => {
				...
				findFirst: async (params: Prisma.PatientFindFirstArgs) =>
					params.where?.id === mockPatient.id ? mockPatient : null,
				findUnique: async (params: Prisma.PatientFindUniqueArgs) =>
					params.where?.id === mockPatient.id ? mockPatient : null,
				create: async (params: Prisma.PatientCreateArgs) => ({
					...mockPatient,
					...params.data,
				}),
				update: async (params: Prisma.PatientUpdateArgs) => ({
					...mockPatient,
					...params.data,
				}),
				delete: async (params: Prisma.PatientDeleteArgs) => ({
					...mockPatient,
					id: params.where.id,
				}),
			},
		...
		patientController = controller(prisma as PrismaClient);
		sentData = undefined;
		statusCode = 200;
		req = {
			query: {},
	...
	describe(".getAll()", () => {
		it("should return paginated patients", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { page: "1", limit: "10" };
			await patientController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
		});

		it("should group patients by type field", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should group patients by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.grouped).to.have.property("User Registration Patient");
			expect(sentData.data.grouped).to.have.property("SMS Notification Patient");
		});

		it("should handle patients with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type" };
			await patientController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
	...
	describe(".getById()", () => {
		it("should return a patient", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockPatient.id };
			await patientController.getById(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.deep.include({ id: mockPatient.id });
		});
		...
		it("should handle non-existent patient", async function () {
			this.timeout(TEST_TIMEOUT);
	...
	describe(".create()", () => {
		it("should create a new patient", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Contact Form Patient",
				description: "Patient for contact forms with validation",
			};
			req.body = createData;
			await patientController.create(req as Request, res, next);
		...
		it("should create a new patient with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email Patient",
				description: "Patient for email notifications",
				type: "email",
			};
			req.body = createData;
			await patientController.create(req as Request, res, next);
		...
		it("should create a new patient without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic Patient",
				description: "Patient without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				name: "Form Patient",
				description: "Patient from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				name: "URL Patient",
				description: "Patient from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle validation errors", async function () {
			...
			const createData = {
				name: "",
				description: "Patient with empty name",
			};
			req.body = createData;
			await patientController.create(req as Request, res, next);
			expect(statusCode).to.equal(400);
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				name: "Test Patient",
				description: "Patient that will cause Prisma error",
			};
			req.body = createData;

			// Mock Prisma to throw an error
			prisma.patient.create = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Patient",
				description: "Patient that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update patient details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Patient",
				description: "Updated patient with additional validation and styling options",
			};
			req.params = { id: mockPatient.id };
			req.body = updateData;
			await patientController.update(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.have.property("id");
		...
		it("should update patient type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple patient fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Email Patient",
		...
		it("should handle non-existent patient update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Patient",
			};
	...
	describe(".remove()", () => {
		it("should delete a patient", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockPatient.id };
			await patientController.remove(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle non-existent patient deletion", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: "507f1f77bcf86cd799439099" };
			await patientController.remove(req as Request, res, next);
			expect(statusCode).to.equal(404);
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long patient name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "A".repeat(1000), // Very long name
				description: "Patient with very long name",
			};
			req.body = createData;
			await patientController.create(req as Request, res, next);
			expect(statusCode).to.equal(201);
		...
		it("should handle special characters in patient data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Patient with special chars: !@#$%^&*()",
		...
		it("should handle concurrent requests", async function () {
			...
			const createData = {
				...
				description: "Patient created concurrently",
			};
			req.body = createData;
			...
			const promises = Array(5)
				.fill(null)
				.map(() => patientController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "Patient 1", type: "email", category: "marketing" },
		{ id: 2, name: "Patient 2", type: "sms", category: "notification" },
		{ id: 3, name: "Patient 3", type: "email", category: "marketing" },
		{ id: 4, name: "Patient 4", type: null, category: "general" },
		{ id: 5, name: "Patient 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "Patient 1", type: "email" },
				{ id: 2, name: "Patient 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("Patient 1");
			expect(result).to.have.property("Patient 2");
			expect(result).to.have.property("Patient 3");
			expect(result).to.have.property("Patient 4");
			expect(result).to.have.property("Patient 5");
			expect(result["Patient 1"]).to.have.length(1);
		});
	});
});
