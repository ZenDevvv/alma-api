...
describe("Item Controller", () => {
	...
	const mockItem = {
		...
		description: "Item for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};
	...
	const mockItems = [
		...
		},
		{
			...
			name: "SMS Notification Item",
			description: "Item for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			name: "Email Marketing Item",
			description: "Item for email marketing campaigns",
			type: "email",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			item: {
				findMany: async (_params: Prisma.ItemFindManyArgs) => {
					// Return multiple items for grouping tests
					if (req.query?.groupBy) {
						return mockItems;
					}
					return [mockItem];
				},
				count: async (_params: Prisma.ItemCountArgs) => {
				...
				findFirst: async (params: Prisma.ItemFindFirstArgs) =>
					params.where?.id === mockItem.id ? mockItem : null,
				findUnique: async (params: Prisma.ItemFindUniqueArgs) =>
					params.where?.id === mockItem.id ? mockItem : null,
				create: async (params: Prisma.ItemCreateArgs) => ({
					...mockItem,
					...params.data,
				}),
				update: async (params: Prisma.ItemUpdateArgs) => ({
					...mockItem,
					...params.data,
				}),
				delete: async (params: Prisma.ItemDeleteArgs) => ({
					...mockItem,
					id: params.where.id,
				}),
			},
	...
	describe(".getAll()", () => {
		...
		it("should group items by type field", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should group items by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.grouped).to.have.property("User Registration Item");
			expect(sentData.data.grouped).to.have.property("SMS Notification Item");
		});

		it("should handle items with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type" };
			await itemController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
	...
	describe(".getById()", () => {
		it("should return a item", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.deep.include({ id: mockItem.id });
		});
		...
		it("should handle non-existent item", async function () {
			this.timeout(TEST_TIMEOUT);
	...
	describe(".create()", () => {
		it("should create a new item", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Contact Form Item",
				description: "Item for contact forms with validation",
			};
			req.body = createData;
		...
		it("should create a new item with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email Item",
				description: "Item for email notifications",
				type: "email",
			};
			req.body = createData;
			await itemController.create(req as Request, res, next);
		...
		it("should create a new item without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic Item",
				description: "Item without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				name: "Form Item",
				description: "Item from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				name: "URL Item",
				description: "Item from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				name: "Test Item",
				description: "Item that will cause Prisma error",
			};
			req.body = createData;

			// Mock Prisma to throw an error
			prisma.item.create = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Item",
				description: "Item that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update item details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Item",
				description: "Updated item with additional validation and styling options",
			};
			req.params = { id: mockItem.id };
			req.body = updateData;
		...
		it("should update item type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple item fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Email Item",
		...
		it("should handle non-existent item update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Item",
			};
	...
	describe(".remove()", () => {
		...
		it("should handle non-existent item deletion", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: "507f1f77bcf86cd799439099" };
			await itemController.remove(req as Request, res, next);
			expect(statusCode).to.equal(404);
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long item name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
		...
		it("should handle special characters in item data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
		...
		it("should handle concurrent requests", async function () {
			...
			const createData = {
				...
				description: "Item created concurrently",
			};
			req.body = createData;
			...
			const promises = Array(5)
				.fill(null)
				.map(() => itemController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "Item 1", type: "email", category: "marketing" },
		{ id: 2, name: "Item 2", type: "sms", category: "notification" },
		{ id: 3, name: "Item 3", type: "email", category: "marketing" },
		{ id: 4, name: "Item 4", type: null, category: "general" },
		{ id: 5, name: "Item 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "Item 1", type: "email" },
				{ id: 2, name: "Item 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("Item 1");
			expect(result).to.have.property("Item 2");
			expect(result).to.have.property("Item 3");
			expect(result).to.have.property("Item 4");
			expect(result).to.have.property("Item 5");
			expect(result["Item 1"]).to.have.length(1);
		});
	});
});
