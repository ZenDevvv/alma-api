import { controller } from "../app/batch/batch.controller";
...
describe("Batch Controller", () => {
	...
	const mockBatch = {
		...
		name: "User Registration Batch",
		description: "Batch for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};
	...
	const mockBatchs = [
		...
		},
		{
			...
			description: "Batch for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "Batch for email marketing campaigns",
			type: "email",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			name: "Generic Batch",
			description: "Batch without type",
			type: null,
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			batch: {
				findMany: async (_params: Prisma.BatchFindManyArgs) => {
					// Return multiple batchs for grouping tests
					if (req.query?.groupBy) {
						return mockBatchs;
					}
					return [mockBatch];
				},
				count: async (_params: Prisma.BatchCountArgs) => {
					// Return count based on whether grouping is requested
					if (req.query?.groupBy) {
						return mockBatchs.length;
					}
					return 1;
				},
				findFirst: async (params: Prisma.BatchFindFirstArgs) =>
					params.where?.id === mockBatch.id ? mockBatch : null,
				findUnique: async (params: Prisma.BatchFindUniqueArgs) =>
					params.where?.id === mockBatch.id ? mockBatch : null,
				create: async (params: Prisma.BatchCreateArgs) => ({
					...mockBatch,
					...params.data,
				}),
				update: async (params: Prisma.BatchUpdateArgs) => ({
					...mockBatch,
					...params.data,
				}),
				delete: async (params: Prisma.BatchDeleteArgs) => ({
					...mockBatch,
					id: params.where.id,
				}),
			},
		...
		batchController = controller(prisma as PrismaClient);
		sentData = undefined;
		statusCode = 200;
		req = {
			query: {},
	...
	describe(".getAll()", () => {
		it("should return paginated batchs", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { page: "1", limit: "10" };
			await batchController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
		});

		it("should group batchs by type field", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should group batchs by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.grouped).to.have.property("User Registration Batch");
			expect(sentData.data.grouped).to.have.property("SMS Notification Batch");
		});

		it("should handle batchs with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type" };
			await batchController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
	...
	describe(".getById()", () => {
		it("should return a batch", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockBatch.id };
			await batchController.getById(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.deep.include({ id: mockBatch.id });
		});
		...
		it("should handle non-existent batch", async function () {
			this.timeout(TEST_TIMEOUT);
	...
	describe(".create()", () => {
		it("should create a new batch", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Contact Form Batch",
				description: "Batch for contact forms with validation",
			};
			req.body = createData;
			await batchController.create(req as Request, res, next);
		...
		it("should create a new batch with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email Batch",
				description: "Batch for email notifications",
				type: "email",
			};
			req.body = createData;
			await batchController.create(req as Request, res, next);
		...
		it("should create a new batch without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic Batch",
				description: "Batch without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				name: "Form Batch",
				description: "Batch from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				name: "URL Batch",
				description: "Batch from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle validation errors", async function () {
			...
			const createData = {
				name: "",
				description: "Batch with empty name",
			};
			req.body = createData;
			await batchController.create(req as Request, res, next);
			expect(statusCode).to.equal(400);
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				name: "Test Batch",
				description: "Batch that will cause Prisma error",
			};
			req.body = createData;

			// Mock Prisma to throw an error
			prisma.batch.create = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Batch",
				description: "Batch that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update batch details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Batch",
				description: "Updated batch with additional validation and styling options",
			};
			req.params = { id: mockBatch.id };
			req.body = updateData;
			await batchController.update(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.have.property("id");
		...
		it("should update batch type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple batch fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Email Batch",
		...
		it("should handle non-existent batch update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Batch",
			};
	...
	describe(".remove()", () => {
		it("should delete a batch", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockBatch.id };
			await batchController.remove(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle non-existent batch deletion", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: "507f1f77bcf86cd799439099" };
			await batchController.remove(req as Request, res, next);
			expect(statusCode).to.equal(404);
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long batch name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "A".repeat(1000), // Very long name
				description: "Batch with very long name",
			};
			req.body = createData;
			await batchController.create(req as Request, res, next);
			expect(statusCode).to.equal(201);
		...
		it("should handle special characters in batch data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Batch with special chars: !@#$%^&*()",
		...
		it("should handle concurrent requests", async function () {
			...
			const createData = {
				...
				description: "Batch created concurrently",
			};
			req.body = createData;
			...
			const promises = Array(5)
				.fill(null)
				.map(() => batchController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "Batch 1", type: "email", category: "marketing" },
		{ id: 2, name: "Batch 2", type: "sms", category: "notification" },
		{ id: 3, name: "Batch 3", type: "email", category: "marketing" },
		{ id: 4, name: "Batch 4", type: null, category: "general" },
		{ id: 5, name: "Batch 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "Batch 1", type: "email" },
				{ id: 2, name: "Batch 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("Batch 1");
			expect(result).to.have.property("Batch 2");
			expect(result).to.have.property("Batch 3");
			expect(result).to.have.property("Batch 4");
			expect(result).to.have.property("Batch 5");
			expect(result["Batch 1"]).to.have.length(1);
		});
	});
});
