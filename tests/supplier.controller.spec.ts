...
describe("Supplier Controller", () => {
	...
	const mockSupplier = {
		...
		name: "User Registration Supplier",
		description: "Supplier for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};
	...
	const mockSuppliers = [
		...
		},
		{
			...
			description: "Supplier for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "Supplier for email marketing campaigns",
			type: "email",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "Supplier without type",
			type: null,
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			supplier: {
				findMany: async (_params: Prisma.SupplierFindManyArgs) => {
					// Return multiple suppliers for grouping tests
					if (req.query?.groupBy) {
						return mockSuppliers;
					}
					return [mockSupplier];
				},
				count: async (_params: Prisma.SupplierCountArgs) => {
				...
				findUnique: async (params: Prisma.SupplierFindUniqueArgs) =>
					params.where?.id === mockSupplier.id ? mockSupplier : null,
	...
	describe(".getAll()", () => {
		...
		it("should group suppliers by type field", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should group suppliers by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.grouped).to.have.property("User Registration Supplier");
			expect(sentData.data.grouped).to.have.property("SMS Notification Supplier");
		});

		it("should handle suppliers with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type" };
			await supplierController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
	...
	describe(".getById()", () => {
		it("should return a supplier", async function () {
			...
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.deep.include({ id: mockSupplier.id });
		});
		...
		it("should handle non-existent supplier", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should handle Prisma errors", async function () {
			...
			// Mock Prisma to throw an error
			prisma.supplier.findUnique = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
	...
	describe(".create()", () => {
		it("should create a new supplier", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Contact Form Supplier",
				description: "Supplier for contact forms with validation",
			};
			req.body = createData;
		...
		it("should create a new supplier with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email Supplier",
				description: "Supplier for email notifications",
				type: "email",
			};
			req.body = createData;
			await supplierController.create(req as Request, res, next);
		...
		it("should create a new supplier without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic Supplier",
				description: "Supplier without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				name: "Form Supplier",
				description: "Supplier from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				name: "URL Supplier",
				description: "Supplier from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle validation errors", async function () {
			...
			const createData = {
				name: "",
				description: "Supplier with empty name",
			};
			req.body = createData;
			await supplierController.create(req as Request, res, next);
			expect(statusCode).to.equal(400);
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				name: "Test Supplier",
				description: "Supplier that will cause Prisma error",
			};
			req.body = createData;

			// Mock Prisma to throw an error
			prisma.supplier.create = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Supplier",
				description: "Supplier that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update supplier details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Supplier",
				description: "Updated supplier with additional validation and styling options",
			};
			req.params = { id: mockSupplier.id };
			req.body = updateData;
			await supplierController.update(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.have.property("id");
		...
		it("should update supplier type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple supplier fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Email Supplier",
		...
		it("should handle non-existent supplier update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Supplier",
			};
	...
	describe(".remove()", () => {
		it("should delete a supplier", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockSupplier.id };
			await supplierController.remove(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle non-existent supplier deletion", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: "507f1f77bcf86cd799439099" };
			await supplierController.remove(req as Request, res, next);
			expect(statusCode).to.equal(404);
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long supplier name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "A".repeat(1000), // Very long name
				description: "Supplier with very long name",
			};
			req.body = createData;
			await supplierController.create(req as Request, res, next);
			expect(statusCode).to.equal(201);
		...
		it("should handle special characters in supplier data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Supplier with special chars: !@#$%^&*()",
		...
		it("should handle concurrent requests", async function () {
			...
			const createData = {
				...
				description: "Supplier created concurrently",
			};
			req.body = createData;
			...
			const promises = Array(5)
				.fill(null)
				.map(() => supplierController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "Supplier 1", type: "email", category: "marketing" },
		{ id: 2, name: "Supplier 2", type: "sms", category: "notification" },
		{ id: 3, name: "Supplier 3", type: "email", category: "marketing" },
		{ id: 4, name: "Supplier 4", type: null, category: "general" },
		{ id: 5, name: "Supplier 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "Supplier 1", type: "email" },
				{ id: 2, name: "Supplier 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("Supplier 1");
			expect(result).to.have.property("Supplier 2");
			expect(result).to.have.property("Supplier 3");
			expect(result).to.have.property("Supplier 4");
			expect(result).to.have.property("Supplier 5");
			expect(result["Supplier 1"]).to.have.length(1);
		});
	});
});
