import { controller } from "../app/metrics/metrics.controller";
...
describe("Metrics Controller", () => {
	...
	const mockMetrics = {
		...
		name: "User Registration Metrics",
		description: "Metrics for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};
	...
	const mockMetricss = [
		...
		},
		{
			...
			description: "Metrics for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		...
		},
		{
			...
			description: "Metrics without type",
			type: null,
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			metrics: {
				findMany: async (_params: Prisma.MetricsFindManyArgs) => {
					// Return multiple metricss for grouping tests
					if (req.query?.groupBy) {
						return mockMetricss;
					}
					return [mockMetrics];
				},
				count: async (_params: Prisma.MetricsCountArgs) => {
					// Return count based on whether grouping is requested
					if (req.query?.groupBy) {
						return mockMetricss.length;
					}
					return 1;
				},
				findFirst: async (params: Prisma.MetricsFindFirstArgs) =>
					params.where?.id === mockMetrics.id ? mockMetrics : null,
				findUnique: async (params: Prisma.MetricsFindUniqueArgs) =>
					params.where?.id === mockMetrics.id ? mockMetrics : null,
				create: async (params: Prisma.MetricsCreateArgs) => ({
					...mockMetrics,
					...params.data,
				}),
				update: async (params: Prisma.MetricsUpdateArgs) => ({
					...mockMetrics,
					...params.data,
				}),
				delete: async (params: Prisma.MetricsDeleteArgs) => ({
					...mockMetrics,
					id: params.where.id,
				}),
			},
		...
		metricsController = controller(prisma as PrismaClient);
		sentData = undefined;
		statusCode = 200;
		req = {
			query: {},
	...
	describe(".getAll()", () => {
		it("should return paginated metricss", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { page: "1", limit: "10" };
			await metricsController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
		});

		it("should group metricss by type field", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should group metricss by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.grouped).to.have.property("User Registration Metrics");
			expect(sentData.data.grouped).to.have.property("SMS Notification Metrics");
		});

		it("should handle metricss with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type" };
			await metricsController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
	...
	describe(".getById()", () => {
		it("should return a metrics", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockMetrics.id };
			await metricsController.getById(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.deep.include({ id: mockMetrics.id });
		});
		...
		it("should handle non-existent metrics", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should handle Prisma errors", async function () {
			...
			// Mock Prisma to throw an error
			prisma.metrics.findUnique = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
	...
	describe(".create()", () => {
		it("should create a new metrics", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Contact Form Metrics",
				description: "Metrics for contact forms with validation",
			};
			req.body = createData;
			await metricsController.create(req as Request, res, next);
		...
		it("should create a new metrics with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email Metrics",
				description: "Metrics for email notifications",
				type: "email",
			};
			req.body = createData;
			await metricsController.create(req as Request, res, next);
		...
		it("should create a new metrics without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic Metrics",
				description: "Metrics without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				name: "Form Metrics",
				description: "Metrics from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				name: "URL Metrics",
				description: "Metrics from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle validation errors", async function () {
			...
			const createData = {
				name: "",
				description: "Metrics with empty name",
			};
			req.body = createData;
			await metricsController.create(req as Request, res, next);
			expect(statusCode).to.equal(400);
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				name: "Test Metrics",
				description: "Metrics that will cause Prisma error",
			};
			req.body = createData;

			// Mock Prisma to throw an error
			prisma.metrics.create = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Metrics",
				description: "Metrics that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update metrics details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Metrics",
				description: "Updated metrics with additional validation and styling options",
			};
			req.params = { id: mockMetrics.id };
			req.body = updateData;
			await metricsController.update(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.have.property("id");
		...
		it("should update metrics type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple metrics fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Email Metrics",
		...
		it("should handle non-existent metrics update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Metrics",
			};
	...
	describe(".remove()", () => {
		it("should delete a metrics", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockMetrics.id };
			await metricsController.remove(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle non-existent metrics deletion", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: "507f1f77bcf86cd799439099" };
			await metricsController.remove(req as Request, res, next);
			expect(statusCode).to.equal(404);
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long metrics name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "A".repeat(1000), // Very long name
				description: "Metrics with very long name",
			};
			req.body = createData;
			await metricsController.create(req as Request, res, next);
			expect(statusCode).to.equal(201);
		...
		it("should handle special characters in metrics data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Metrics with special chars: !@#$%^&*()",
		...
		it("should handle concurrent requests", async function () {
			...
			const createData = {
				...
				description: "Metrics created concurrently",
			};
			req.body = createData;
			...
			const promises = Array(5)
				.fill(null)
				.map(() => metricsController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "Metrics 1", type: "email", category: "marketing" },
		{ id: 2, name: "Metrics 2", type: "sms", category: "notification" },
		{ id: 3, name: "Metrics 3", type: "email", category: "marketing" },
		{ id: 4, name: "Metrics 4", type: null, category: "general" },
		{ id: 5, name: "Metrics 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "Metrics 1", type: "email" },
				{ id: 2, name: "Metrics 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("Metrics 1");
			expect(result).to.have.property("Metrics 2");
			expect(result).to.have.property("Metrics 3");
			expect(result).to.have.property("Metrics 4");
			expect(result).to.have.property("Metrics 5");
			expect(result["Metrics 1"]).to.have.length(1);
		});
	});
});
