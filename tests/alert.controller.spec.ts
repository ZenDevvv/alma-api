import { controller } from "../app/alert/alert.controller";
...
describe("Alert Controller", () => {
	...
	const mockAlert = {
		...
		name: "User Registration Alert",
		description: "Alert for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};
	...
	const mockAlerts = [
		...
		},
		{
			...
			description: "Alert for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			name: "Email Marketing Alert",
			description: "Alert for email marketing campaigns",
			type: "email",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			name: "Generic Alert",
			description: "Alert without type",
			type: null,
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			alert: {
				findMany: async (_params: Prisma.AlertFindManyArgs) => {
					// Return multiple alerts for grouping tests
					if (req.query?.groupBy) {
						return mockAlerts;
					}
					return [mockAlert];
				},
				count: async (_params: Prisma.AlertCountArgs) => {
					// Return count based on whether grouping is requested
					if (req.query?.groupBy) {
						return mockAlerts.length;
					}
					return 1;
				},
				findFirst: async (params: Prisma.AlertFindFirstArgs) =>
					params.where?.id === mockAlert.id ? mockAlert : null,
				findUnique: async (params: Prisma.AlertFindUniqueArgs) =>
					params.where?.id === mockAlert.id ? mockAlert : null,
				create: async (params: Prisma.AlertCreateArgs) => ({
					...mockAlert,
					...params.data,
				}),
				update: async (params: Prisma.AlertUpdateArgs) => ({
					...mockAlert,
					...params.data,
				}),
				delete: async (params: Prisma.AlertDeleteArgs) => ({
					...mockAlert,
					id: params.where.id,
				}),
			},
		...
		alertController = controller(prisma as PrismaClient);
		sentData = undefined;
		statusCode = 200;
		req = {
			query: {},
	...
	describe(".getAll()", () => {
		it("should return paginated alerts", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { page: "1", limit: "10" };
			await alertController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
		});

		it("should group alerts by type field", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should group alerts by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.grouped).to.have.property("User Registration Alert");
			expect(sentData.data.grouped).to.have.property("SMS Notification Alert");
		});

		it("should handle alerts with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type" };
			await alertController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
	...
	describe(".getById()", () => {
		it("should return a alert", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockAlert.id };
			await alertController.getById(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.deep.include({ id: mockAlert.id });
		});
	...
	describe(".create()", () => {
		it("should create a new alert", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Contact Form Alert",
				description: "Alert for contact forms with validation",
			};
			req.body = createData;
			await alertController.create(req as Request, res, next);
		...
		it("should create a new alert with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email Alert",
				description: "Alert for email notifications",
				type: "email",
			};
			req.body = createData;
			await alertController.create(req as Request, res, next);
		...
		it("should create a new alert without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic Alert",
				description: "Alert without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				name: "Form Alert",
				description: "Alert from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				name: "URL Alert",
				description: "Alert from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle validation errors", async function () {
			...
			const createData = {
				name: "",
				description: "Alert with empty name",
			};
			req.body = createData;
			await alertController.create(req as Request, res, next);
			expect(statusCode).to.equal(400);
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				name: "Test Alert",
				description: "Alert that will cause Prisma error",
			};
			req.body = createData;

			// Mock Prisma to throw an error
			prisma.alert.create = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Alert",
				description: "Alert that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update alert details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Alert",
				description: "Updated alert with additional validation and styling options",
			};
			req.params = { id: mockAlert.id };
			req.body = updateData;
			await alertController.update(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.have.property("id");
		...
		it("should update alert type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple alert fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
		...
		it("should handle non-existent alert update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Alert",
			};
	...
	describe(".remove()", () => {
		it("should delete a alert", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockAlert.id };
			await alertController.remove(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle non-existent alert deletion", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: "507f1f77bcf86cd799439099" };
			await alertController.remove(req as Request, res, next);
			expect(statusCode).to.equal(404);
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long alert name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "A".repeat(1000), // Very long name
				description: "Alert with very long name",
			};
			req.body = createData;
			await alertController.create(req as Request, res, next);
			expect(statusCode).to.equal(201);
		...
		it("should handle special characters in alert data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
		...
		it("should handle concurrent requests", async function () {
			...
			const createData = {
				...
				description: "Alert created concurrently",
			};
			req.body = createData;
			...
			const promises = Array(5)
				.fill(null)
				.map(() => alertController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "Alert 1", type: "email", category: "marketing" },
		{ id: 2, name: "Alert 2", type: "sms", category: "notification" },
		{ id: 3, name: "Alert 3", type: "email", category: "marketing" },
		{ id: 4, name: "Alert 4", type: null, category: "general" },
		{ id: 5, name: "Alert 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "Alert 1", type: "email" },
				{ id: 2, name: "Alert 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("Alert 1");
			expect(result).to.have.property("Alert 2");
			expect(result).to.have.property("Alert 3");
			expect(result).to.have.property("Alert 4");
			expect(result).to.have.property("Alert 5");
			expect(result["Alert 1"]).to.have.length(1);
		});
	});
});
