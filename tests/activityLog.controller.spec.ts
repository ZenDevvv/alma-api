import { controller } from "../app/activityLog/activityLog.controller";
...
describe("ActivityLog Controller", () => {
	...
	const mockActivityLog = {
		...
		name: "User Registration ActivityLog",
		description: "ActivityLog for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};
	...
	const mockActivityLogs = [
		...
		},
		{
			...
			description: "ActivityLog for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "ActivityLog for email marketing campaigns",
			type: "email",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "ActivityLog without type",
			type: null,
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			activityLog: {
				findMany: async (_params: Prisma.ActivityLogFindManyArgs) => {
					// Return multiple activityLogs for grouping tests
					if (req.query?.groupBy) {
						return mockActivityLogs;
					}
					return [mockActivityLog];
				},
				count: async (_params: Prisma.ActivityLogCountArgs) => {
					// Return count based on whether grouping is requested
					if (req.query?.groupBy) {
						return mockActivityLogs.length;
					}
					return 1;
				},
				findFirst: async (params: Prisma.ActivityLogFindFirstArgs) =>
					params.where?.id === mockActivityLog.id ? mockActivityLog : null,
				findUnique: async (params: Prisma.ActivityLogFindUniqueArgs) =>
					params.where?.id === mockActivityLog.id ? mockActivityLog : null,
				create: async (params: Prisma.ActivityLogCreateArgs) => ({
					...mockActivityLog,
					...params.data,
				}),
				update: async (params: Prisma.ActivityLogUpdateArgs) => ({
					...mockActivityLog,
					...params.data,
				}),
				delete: async (params: Prisma.ActivityLogDeleteArgs) => ({
					...mockActivityLog,
					id: params.where.id,
				}),
			},
		...
		activityLogController = controller(prisma as PrismaClient);
		sentData = undefined;
		statusCode = 200;
		req = {
			query: {},
	...
	describe(".getAll()", () => {
		it("should return paginated activityLogs", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { page: "1", limit: "10" };
			await activityLogController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
		});

		it("should group activityLogs by type field", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should group activityLogs by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.grouped).to.have.property("User Registration ActivityLog");
			expect(sentData.data.grouped).to.have.property("SMS Notification ActivityLog");
		});

		it("should handle activityLogs with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type" };
			await activityLogController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
		...
		it("should handle Prisma errors", async function () {
			...
			prisma.activityLog.findMany = async () => {
	...
	describe(".getById()", () => {
		it("should return a activityLog", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockActivityLog.id };
			await activityLogController.getById(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.deep.include({ id: mockActivityLog.id });
		});
		...
		it("should handle non-existent activityLog", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should handle Prisma errors", async function () {
			...
			// Mock Prisma to throw an error
			prisma.activityLog.findUnique = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
	...
	describe(".create()", () => {
		it("should create a new activityLog", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Contact Form ActivityLog",
				description: "ActivityLog for contact forms with validation",
			};
			req.body = createData;
			await activityLogController.create(req as Request, res, next);
		...
		it("should create a new activityLog with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email ActivityLog",
				description: "ActivityLog for email notifications",
				type: "email",
			};
			req.body = createData;
			await activityLogController.create(req as Request, res, next);
		...
		it("should create a new activityLog without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic ActivityLog",
				description: "ActivityLog without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				...
				description: "ActivityLog from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "URL ActivityLog",
				description: "ActivityLog from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle validation errors", async function () {
			...
			const createData = {
				name: "",
				description: "ActivityLog with empty name",
			};
			req.body = createData;
			await activityLogController.create(req as Request, res, next);
			expect(statusCode).to.equal(400);
		...
		it("should handle Prisma errors", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Test ActivityLog",
				description: "ActivityLog that will cause Prisma error",
			};
			req.body = createData;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test ActivityLog",
				description: "ActivityLog that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update activityLog details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form ActivityLog",
				description: "Updated activityLog with additional validation and styling options",
			};
			req.params = { id: mockActivityLog.id };
			req.body = updateData;
			await activityLogController.update(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.have.property("id");
		...
		it("should update activityLog type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple activityLog fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Email ActivityLog",
		...
		it("should handle non-existent activityLog update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated ActivityLog",
			};
		...
		it("should handle Prisma errors", async function () {
			...
			// Mock Prisma to throw an error
			prisma.activityLog.update = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
	...
	describe(".remove()", () => {
		it("should delete a activityLog", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockActivityLog.id };
			await activityLogController.remove(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle non-existent activityLog deletion", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: "507f1f77bcf86cd799439099" };
			await activityLogController.remove(req as Request, res, next);
			expect(statusCode).to.equal(404);
		...
		it("should handle Prisma errors", async function () {
			...
			// Mock Prisma to throw an error
			prisma.activityLog.delete = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long activityLog name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "A".repeat(1000), // Very long name
				description: "ActivityLog with very long name",
			};
			req.body = createData;
			await activityLogController.create(req as Request, res, next);
			expect(statusCode).to.equal(201);
		...
		it("should handle special characters in activityLog data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "ActivityLog with special chars: !@#$%^&*()",
		...
		it("should handle concurrent requests", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Concurrent ActivityLog",
				description: "ActivityLog created concurrently",
			};
			req.body = createData;
			...
			const promises = Array(5)
				.fill(null)
				.map(() => activityLogController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "ActivityLog 1", type: "email", category: "marketing" },
		{ id: 2, name: "ActivityLog 2", type: "sms", category: "notification" },
		{ id: 3, name: "ActivityLog 3", type: "email", category: "marketing" },
		{ id: 4, name: "ActivityLog 4", type: null, category: "general" },
		{ id: 5, name: "ActivityLog 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "ActivityLog 1", type: "email" },
				{ id: 2, name: "ActivityLog 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("ActivityLog 1");
			expect(result).to.have.property("ActivityLog 2");
			expect(result).to.have.property("ActivityLog 3");
			expect(result).to.have.property("ActivityLog 4");
			expect(result).to.have.property("ActivityLog 5");
			expect(result["ActivityLog 1"]).to.have.length(1);
		});
	});
});
