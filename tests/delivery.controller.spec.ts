import { controller } from "../app/delivery/delivery.controller";
...
describe("Delivery Controller", () => {
	...
	const mockDelivery = {
		...
		name: "User Registration Delivery",
		description: "Delivery for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};
	...
	const mockDeliverys = [
		...
		},
		{
			...
			description: "Delivery for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "Delivery for email marketing campaigns",
			type: "email",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "Delivery without type",
			type: null,
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			delivery: {
				findMany: async (_params: Prisma.DeliveryFindManyArgs) => {
					// Return multiple deliverys for grouping tests
					if (req.query?.groupBy) {
						return mockDeliverys;
					}
					return [mockDelivery];
				},
				count: async (_params: Prisma.DeliveryCountArgs) => {
					// Return count based on whether grouping is requested
					if (req.query?.groupBy) {
						return mockDeliverys.length;
					}
					return 1;
				},
				findFirst: async (params: Prisma.DeliveryFindFirstArgs) =>
					params.where?.id === mockDelivery.id ? mockDelivery : null,
				findUnique: async (params: Prisma.DeliveryFindUniqueArgs) =>
					params.where?.id === mockDelivery.id ? mockDelivery : null,
				create: async (params: Prisma.DeliveryCreateArgs) => ({
					...mockDelivery,
					...params.data,
				}),
				update: async (params: Prisma.DeliveryUpdateArgs) => ({
					...mockDelivery,
					...params.data,
				}),
				delete: async (params: Prisma.DeliveryDeleteArgs) => ({
					...mockDelivery,
					id: params.where.id,
				}),
			},
		...
		deliveryController = controller(prisma as PrismaClient);
		sentData = undefined;
		statusCode = 200;
		req = {
			query: {},
	...
	describe(".getAll()", () => {
		it("should return paginated deliverys", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { page: "1", limit: "10" };
			await deliveryController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
		});

		it("should group deliverys by type field", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should group deliverys by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.grouped).to.have.property("User Registration Delivery");
			expect(sentData.data.grouped).to.have.property("SMS Notification Delivery");
		});

		it("should handle deliverys with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type" };
			await deliveryController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
		...
		it("should handle Prisma errors", async function () {
			...
			prisma.delivery.findMany = async () => {
	...
	describe(".getById()", () => {
		it("should return a delivery", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockDelivery.id };
			await deliveryController.getById(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.deep.include({ id: mockDelivery.id });
		});
		...
		it("should handle non-existent delivery", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should handle Prisma errors", async function () {
			...
			// Mock Prisma to throw an error
			prisma.delivery.findUnique = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
	...
	describe(".create()", () => {
		it("should create a new delivery", async function () {
			...
			const createData = {
				...
				description: "Delivery for contact forms with validation",
			};
			req.body = createData;
			await deliveryController.create(req as Request, res, next);
		...
		it("should create a new delivery with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email Delivery",
				description: "Delivery for email notifications",
				type: "email",
			};
			req.body = createData;
			await deliveryController.create(req as Request, res, next);
		...
		it("should create a new delivery without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic Delivery",
				description: "Delivery without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				name: "Form Delivery",
				description: "Delivery from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				name: "URL Delivery",
				description: "Delivery from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle validation errors", async function () {
			...
			const createData = {
				name: "",
				description: "Delivery with empty name",
			};
			req.body = createData;
			await deliveryController.create(req as Request, res, next);
			expect(statusCode).to.equal(400);
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				name: "Test Delivery",
				description: "Delivery that will cause Prisma error",
			};
			req.body = createData;

			// Mock Prisma to throw an error
			prisma.delivery.create = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Delivery",
				description: "Delivery that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update delivery details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Delivery",
				description: "Updated delivery with additional validation and styling options",
			};
			req.params = { id: mockDelivery.id };
			req.body = updateData;
			await deliveryController.update(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.have.property("id");
		...
		it("should update delivery type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple delivery fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
		...
		it("should handle non-existent delivery update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Delivery",
			};
	...
	describe(".remove()", () => {
		it("should delete a delivery", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockDelivery.id };
			await deliveryController.remove(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle non-existent delivery deletion", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: "507f1f77bcf86cd799439099" };
			await deliveryController.remove(req as Request, res, next);
			expect(statusCode).to.equal(404);
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long delivery name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "A".repeat(1000), // Very long name
				description: "Delivery with very long name",
			};
			req.body = createData;
			await deliveryController.create(req as Request, res, next);
			expect(statusCode).to.equal(201);
		...
		it("should handle special characters in delivery data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Delivery with special chars: !@#$%^&*()",
		...
		it("should handle concurrent requests", async function () {
			...
			const createData = {
				...
				description: "Delivery created concurrently",
			};
			req.body = createData;
			...
			const promises = Array(5)
				.fill(null)
				.map(() => deliveryController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "Delivery 1", type: "email", category: "marketing" },
		{ id: 2, name: "Delivery 2", type: "sms", category: "notification" },
		{ id: 3, name: "Delivery 3", type: "email", category: "marketing" },
		{ id: 4, name: "Delivery 4", type: null, category: "general" },
		{ id: 5, name: "Delivery 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "Delivery 1", type: "email" },
				{ id: 2, name: "Delivery 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("Delivery 1");
			expect(result).to.have.property("Delivery 2");
			expect(result).to.have.property("Delivery 3");
			expect(result).to.have.property("Delivery 4");
			expect(result).to.have.property("Delivery 5");
			expect(result["Delivery 1"]).to.have.length(1);
		});
	});
});
