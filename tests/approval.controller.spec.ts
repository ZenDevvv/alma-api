import { controller } from "../app/approval/approval.controller";
...
describe("Approval Controller", () => {
	...
	const mockApproval = {
		...
		description: "Approval for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};
	...
	const mockApprovals = [
		...
		},
		{
			...
			description: "Approval for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "Approval for email marketing campaigns",
			type: "email",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "Approval without type",
			type: null,
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			approval: {
				findMany: async (_params: Prisma.ApprovalFindManyArgs) => {
					// Return multiple approvals for grouping tests
					if (req.query?.groupBy) {
						return mockApprovals;
					}
					return [mockApproval];
				},
				count: async (_params: Prisma.ApprovalCountArgs) => {
				...
				findFirst: async (params: Prisma.ApprovalFindFirstArgs) =>
					params.where?.id === mockApproval.id ? mockApproval : null,
				findUnique: async (params: Prisma.ApprovalFindUniqueArgs) =>
					params.where?.id === mockApproval.id ? mockApproval : null,
				create: async (params: Prisma.ApprovalCreateArgs) => ({
					...mockApproval,
					...params.data,
				}),
				update: async (params: Prisma.ApprovalUpdateArgs) => ({
					...mockApproval,
					...params.data,
				}),
				delete: async (params: Prisma.ApprovalDeleteArgs) => ({
					...mockApproval,
					id: params.where.id,
				}),
			},
		...
		approvalController = controller(prisma as PrismaClient);
		sentData = undefined;
		statusCode = 200;
		req = {
			query: {},
	...
	describe(".getAll()", () => {
		it("should return paginated approvals", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { page: "1", limit: "10" };
			await approvalController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
		});

		it("should group approvals by type field", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should group approvals by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.grouped).to.have.property("User Registration Approval");
			expect(sentData.data.grouped).to.have.property("SMS Notification Approval");
		});

		it("should handle approvals with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type" };
			await approvalController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
		...
		it("should handle Prisma errors", async function () {
			...
			prisma.approval.findMany = async () => {
	...
	describe(".getById()", () => {
		it("should return a approval", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockApproval.id };
			await approvalController.getById(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.deep.include({ id: mockApproval.id });
		});
		...
		it("should handle non-existent approval", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should handle Prisma errors", async function () {
			...
			// Mock Prisma to throw an error
			prisma.approval.findUnique = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
	...
	describe(".create()", () => {
		it("should create a new approval", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Contact Form Approval",
				description: "Approval for contact forms with validation",
			};
			req.body = createData;
			await approvalController.create(req as Request, res, next);
		...
		it("should create a new approval with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email Approval",
				description: "Approval for email notifications",
				type: "email",
			};
			req.body = createData;
			await approvalController.create(req as Request, res, next);
		...
		it("should create a new approval without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic Approval",
				description: "Approval without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				name: "Form Approval",
				description: "Approval from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				name: "URL Approval",
				description: "Approval from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle validation errors", async function () {
			...
			const createData = {
				name: "",
				description: "Approval with empty name",
			};
			req.body = createData;
			await approvalController.create(req as Request, res, next);
			expect(statusCode).to.equal(400);
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				name: "Test Approval",
				description: "Approval that will cause Prisma error",
			};
			req.body = createData;

			// Mock Prisma to throw an error
			prisma.approval.create = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Approval",
				description: "Approval that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update approval details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Approval",
				description: "Updated approval with additional validation and styling options",
			};
			req.params = { id: mockApproval.id };
			req.body = updateData;
			await approvalController.update(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.have.property("id");
		...
		it("should update approval type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple approval fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Email Approval",
		...
		it("should handle non-existent approval update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Approval",
			};
	...
	describe(".remove()", () => {
		it("should delete a approval", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockApproval.id };
			await approvalController.remove(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle non-existent approval deletion", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: "507f1f77bcf86cd799439099" };
			await approvalController.remove(req as Request, res, next);
			expect(statusCode).to.equal(404);
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long approval name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "A".repeat(1000), // Very long name
				description: "Approval with very long name",
			};
			req.body = createData;
			await approvalController.create(req as Request, res, next);
			expect(statusCode).to.equal(201);
		...
		it("should handle special characters in approval data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Approval with special chars: !@#$%^&*()",
		...
		it("should handle concurrent requests", async function () {
			...
			const createData = {
				...
				description: "Approval created concurrently",
			};
			req.body = createData;
			...
			const promises = Array(5)
				.fill(null)
				.map(() => approvalController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "Approval 1", type: "email", category: "marketing" },
		{ id: 2, name: "Approval 2", type: "sms", category: "notification" },
		{ id: 3, name: "Approval 3", type: "email", category: "marketing" },
		{ id: 4, name: "Approval 4", type: null, category: "general" },
		{ id: 5, name: "Approval 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "Approval 1", type: "email" },
				{ id: 2, name: "Approval 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("Approval 1");
			expect(result).to.have.property("Approval 2");
			expect(result).to.have.property("Approval 3");
			expect(result).to.have.property("Approval 4");
			expect(result).to.have.property("Approval 5");
			expect(result["Approval 1"]).to.have.length(1);
		});
	});
});
