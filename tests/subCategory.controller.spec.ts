import { controller } from "../app/subCategory/subCategory.controller";
...
describe("SubCategory Controller", () => {
	...
	const mockSubCategory = {
		...
		name: "User Registration SubCategory",
		description: "SubCategory for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};

	const mockSubCategorys = [
		{
			id: "507f1f77bcf86cd799439026",
			name: "User Registration SubCategory",
			description: "SubCategory for user registration forms",
		...
		},
		{
			...
			description: "SubCategory for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "SubCategory for email marketing campaigns",
			type: "email",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "SubCategory without type",
			type: null,
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			subCategory: {
				findMany: async (_params: Prisma.SubCategoryFindManyArgs) => {
					// Return multiple subCategorys for grouping tests
					if (req.query?.groupBy) {
						return mockSubCategorys;
					}
					return [mockSubCategory];
				},
				count: async (_params: Prisma.SubCategoryCountArgs) => {
					// Return count based on whether grouping is requested
					if (req.query?.groupBy) {
						return mockSubCategorys.length;
					}
					return 1;
				},
				findFirst: async (params: Prisma.SubCategoryFindFirstArgs) =>
					params.where?.id === mockSubCategory.id ? mockSubCategory : null,
				findUnique: async (params: Prisma.SubCategoryFindUniqueArgs) =>
					params.where?.id === mockSubCategory.id ? mockSubCategory : null,
				create: async (params: Prisma.SubCategoryCreateArgs) => ({
					...mockSubCategory,
					...params.data,
				}),
				update: async (params: Prisma.SubCategoryUpdateArgs) => ({
					...mockSubCategory,
					...params.data,
				}),
				delete: async (params: Prisma.SubCategoryDeleteArgs) => ({
					...mockSubCategory,
					id: params.where.id,
				}),
			},
		...
		subCategoryController = controller(prisma as PrismaClient);
		sentData = undefined;
		statusCode = 200;
		req = {
			query: {},
	...
	describe(".getAll()", () => {
		it("should return paginated subCategorys", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { page: "1", limit: "10" };
			await subCategoryController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
		});

		it("should group subCategorys by type field", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should group subCategorys by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.grouped).to.have.property("User Registration SubCategory");
			expect(sentData.data.grouped).to.have.property("SMS Notification SubCategory");
		});

		it("should handle subCategorys with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type" };
			await subCategoryController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
		...
		it("should handle Prisma errors", async function () {
			...
			prisma.subCategory.findMany = async () => {
	...
	describe(".getById()", () => {
		it("should return a subCategory", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockSubCategory.id };
			await subCategoryController.getById(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.deep.include({ id: mockSubCategory.id });
		});
		...
		it("should handle non-existent subCategory", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should handle Prisma errors", async function () {
			...
			// Mock Prisma to throw an error
			prisma.subCategory.findUnique = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
	...
	describe(".create()", () => {
		it("should create a new subCategory", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Contact Form SubCategory",
				description: "SubCategory for contact forms with validation",
			};
			req.body = createData;
			await subCategoryController.create(req as Request, res, next);
		...
		it("should create a new subCategory with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email SubCategory",
				description: "SubCategory for email notifications",
				type: "email",
			};
			req.body = createData;
			await subCategoryController.create(req as Request, res, next);
		...
		it("should create a new subCategory without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic SubCategory",
				description: "SubCategory without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				...
				description: "SubCategory from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "URL SubCategory",
				description: "SubCategory from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle validation errors", async function () {
			...
			const createData = {
				name: "",
				description: "SubCategory with empty name",
			};
			req.body = createData;
			await subCategoryController.create(req as Request, res, next);
			expect(statusCode).to.equal(400);
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				...
				description: "SubCategory that will cause Prisma error",
			};
			req.body = createData;

			// Mock Prisma to throw an error
			prisma.subCategory.create = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test SubCategory",
				description: "SubCategory that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update subCategory details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form SubCategory",
				description: "Updated subCategory with additional validation and styling options",
			};
			req.params = { id: mockSubCategory.id };
			req.body = updateData;
			await subCategoryController.update(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.have.property("id");
		...
		it("should update subCategory type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple subCategory fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Email SubCategory",
		...
		it("should handle non-existent subCategory update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated SubCategory",
			};
		...
		it("should handle Prisma errors", async function () {
			...
			// Mock Prisma to throw an error
			prisma.subCategory.update = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
	...
	describe(".remove()", () => {
		it("should delete a subCategory", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockSubCategory.id };
			await subCategoryController.remove(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle non-existent subCategory deletion", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: "507f1f77bcf86cd799439099" };
			await subCategoryController.remove(req as Request, res, next);
			expect(statusCode).to.equal(404);
		...
		it("should handle Prisma errors", async function () {
			...
			// Mock Prisma to throw an error
			prisma.subCategory.delete = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long subCategory name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "A".repeat(1000), // Very long name
				description: "SubCategory with very long name",
			};
			req.body = createData;
			await subCategoryController.create(req as Request, res, next);
			expect(statusCode).to.equal(201);
		...
		it("should handle special characters in subCategory data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "SubCategory with special chars: !@#$%^&*()",
		...
		it("should handle concurrent requests", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Concurrent SubCategory",
				description: "SubCategory created concurrently",
			};
			req.body = createData;
			...
			const promises = Array(5)
				.fill(null)
				.map(() => subCategoryController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "SubCategory 1", type: "email", category: "marketing" },
		{ id: 2, name: "SubCategory 2", type: "sms", category: "notification" },
		{ id: 3, name: "SubCategory 3", type: "email", category: "marketing" },
		{ id: 4, name: "SubCategory 4", type: null, category: "general" },
		{ id: 5, name: "SubCategory 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "SubCategory 1", type: "email" },
				{ id: 2, name: "SubCategory 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("SubCategory 1");
			expect(result).to.have.property("SubCategory 2");
			expect(result).to.have.property("SubCategory 3");
			expect(result).to.have.property("SubCategory 4");
			expect(result).to.have.property("SubCategory 5");
			expect(result["SubCategory 1"]).to.have.length(1);
		});
	});
});
