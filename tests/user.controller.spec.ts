...
describe("User Controller", () => {
	...
	const mockUser = {
		...
		description: "User for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};
	...
	const mockUsers = [
		...
		},
		{
			...
			description: "User for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "User for email marketing campaigns",
			type: "email",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			user: {
				findMany: async (_params: Prisma.UserFindManyArgs) => {
					// Return multiple users for grouping tests
					if (req.query?.groupBy) {
						return mockUsers;
					}
					return [mockUser];
				},
				count: async (_params: Prisma.UserCountArgs) => {
				...
				findUnique: async (params: Prisma.UserFindUniqueArgs) =>
					params.where?.id === mockUser.id ? mockUser : null,
				create: async (params: Prisma.UserCreateArgs) => ({
					...mockUser,
					...params.data,
				}),
				...
				delete: async (params: Prisma.UserDeleteArgs) => ({
					...mockUser,
					id: params.where.id,
				}),
			},
	...
	describe(".getAll()", () => {
		...
		it("should group users by type field", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should group users by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.grouped).to.have.property("User Registration User");
			expect(sentData.data.grouped).to.have.property("SMS Notification User");
		});

		it("should handle users with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type" };
			await userController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
	...
	describe(".create()", () => {
		it("should create a new user", async function () {
			...
			const createData = {
				...
				description: "User for contact forms with validation",
			};
			req.body = createData;
			await userController.create(req as Request, res, next);
		...
		it("should create a new user with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email User",
				description: "User for email notifications",
				type: "email",
			};
			req.body = createData;
			await userController.create(req as Request, res, next);
		...
		it("should create a new user without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic User",
				description: "User without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				name: "Form User",
				description: "User from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				name: "URL User",
				description: "User from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				name: "Test User",
				description: "User that will cause Prisma error",
			};
			req.body = createData;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test User",
				description: "User that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update user details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form User",
				description: "Updated user with additional validation and styling options",
			};
			req.params = { id: mockUser.id };
			req.body = updateData;
		...
		it("should update user type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple user fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
		...
		it("should handle non-existent user update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated User",
			};
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long user name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "A".repeat(1000), // Very long name
				description: "User with very long name",
			};
			req.body = createData;
			await userController.create(req as Request, res, next);
			expect(statusCode).to.equal(201);
		...
		it("should handle special characters in user data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
		...
		it("should handle concurrent requests", async function () {
			...
			const createData = {
				...
				description: "User created concurrently",
			};
			req.body = createData;
			...
			const promises = Array(5)
				.fill(null)
				.map(() => userController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "User 1", type: "email", category: "marketing" },
		{ id: 2, name: "User 2", type: "sms", category: "notification" },
		{ id: 3, name: "User 3", type: "email", category: "marketing" },
		{ id: 4, name: "User 4", type: null, category: "general" },
		{ id: 5, name: "User 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "User 1", type: "email" },
				{ id: 2, name: "User 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("User 1");
			expect(result).to.have.property("User 2");
			expect(result).to.have.property("User 3");
			expect(result).to.have.property("User 4");
			expect(result).to.have.property("User 5");
			expect(result["User 1"]).to.have.length(1);
		});
	});
});
