...
describe("Product Controller", () => {
	...
	const mockProduct = {
		...
		name: "User Registration Product",
		description: "Product for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};
	...
	const mockProducts = [
		...
		},
		{
			...
			name: "SMS Notification Product",
			description: "Product for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "Product for email marketing campaigns",
			type: "email",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "Product without type",
			type: null,
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			product: {
				findMany: async (_params: Prisma.ProductFindManyArgs) => {
					// Return multiple products for grouping tests
					if (req.query?.groupBy) {
						return mockProducts;
					}
					return [mockProduct];
				},
				count: async (_params: Prisma.ProductCountArgs) => {
				...
				findFirst: async (params: Prisma.ProductFindFirstArgs) =>
					params.where?.id === mockProduct.id ? mockProduct : null,
				findUnique: async (params: Prisma.ProductFindUniqueArgs) =>
					params.where?.id === mockProduct.id ? mockProduct : null,
				create: async (params: Prisma.ProductCreateArgs) => ({
					...mockProduct,
					...params.data,
				}),
				update: async (params: Prisma.ProductUpdateArgs) => ({
					...mockProduct,
					...params.data,
				}),
				delete: async (params: Prisma.ProductDeleteArgs) => ({
					...mockProduct,
					id: params.where.id,
				}),
			},
	...
	describe(".getAll()", () => {
		...
		it("should group products by type field", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should group products by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.grouped).to.have.property("User Registration Product");
			expect(sentData.data.grouped).to.have.property("SMS Notification Product");
		});

		it("should handle products with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type" };
			await productController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
	...
	describe(".create()", () => {
		it("should create a new product", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Contact Form Product",
				description: "Product for contact forms with validation",
			};
			req.body = createData;
		...
		it("should create a new product with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email Product",
				description: "Product for email notifications",
				type: "email",
			};
			req.body = createData;
			await productController.create(req as Request, res, next);
		...
		it("should create a new product without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic Product",
				description: "Product without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				name: "Form Product",
				description: "Product from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				name: "URL Product",
				description: "Product from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				name: "Test Product",
				description: "Product that will cause Prisma error",
			};
			req.body = createData;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Product",
				description: "Product that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update product details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Product",
				description: "Updated product with additional validation and styling options",
			};
			req.params = { id: mockProduct.id };
			req.body = updateData;
		...
		it("should update product type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple product fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Email Product",
		...
		it("should handle non-existent product update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Product",
			};
	...
	describe(".remove()", () => {
		...
		it("should handle non-existent product deletion", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: "507f1f77bcf86cd799439099" };
			await productController.remove(req as Request, res, next);
			expect(statusCode).to.equal(404);
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long product name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
		...
		it("should handle special characters in product data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
		...
		it("should handle concurrent requests", async function () {
			...
			const createData = {
				...
				description: "Product created concurrently",
			};
			req.body = createData;
			...
			const promises = Array(5)
				.fill(null)
				.map(() => productController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "Product 1", type: "email", category: "marketing" },
		{ id: 2, name: "Product 2", type: "sms", category: "notification" },
		{ id: 3, name: "Product 3", type: "email", category: "marketing" },
		{ id: 4, name: "Product 4", type: null, category: "general" },
		{ id: 5, name: "Product 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "Product 1", type: "email" },
				{ id: 2, name: "Product 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("Product 1");
			expect(result).to.have.property("Product 2");
			expect(result).to.have.property("Product 3");
			expect(result).to.have.property("Product 4");
			expect(result).to.have.property("Product 5");
			expect(result["Product 1"]).to.have.length(1);
		});
	});
});
