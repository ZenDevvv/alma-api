...
export const router = (route: Router, controller: IController): Router => {
	 ...
	 *     description: Retrieve a specific medicine by its unique identifier with optional field selection
	 *     tags: [Medicine]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *         description: Medicine ID (MongoDB ObjectId format)
	 ...
	 */
	// Cache individual medicine with predictable key for invalidation
	routes.get(
		"/:id",
		cache({
			ttl: 90,
			keyGenerator: (req: Request) => {
				...
				return `cache:medicine:byId:${req.params.id}:${fields}`;
			},
		}),
		controller.getById,
	);
	 ...
	 *     description: Retrieve medicines with advanced filtering, pagination, sorting, field selection, and optional grouping
	 *     tags: [Medicine]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *           type: string
	 *           enum: ["true"]
	 *         description: Include medicine documents in response
	 *       - in: query
	 *         name: pagination
	 *         required: false
	 *         schema:
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	// Cache medicine list with predictable key for invalidation
	routes.get(
		"/",
		cache({
			ttl: 60,
			keyGenerator: (req: Request) => {
				...
				return `cache:medicine:list:${queryKey}`;
			},
		}),
		controller.getAll,
	);
	 ...
	 *     description: Create a new medicine with the provided data
	 *     tags: [Medicine]
	 *     security:
	 *       - bearerAuth: []
	 *     requestBody:
	 ...
	 *                 example: "Email Welcome Medicine"
	 *               description:
	 *                 type: string
	 *                 description: Medicine description
	 *                 example: "Welcome email medicine for new users"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: Medicine type for categorization
	 *                 example: "email"
	 *               isDeleted:
	 *                 type: boolean
	 ...
	 *     description: Update medicine data by ID (partial update)
	 *     tags: [Medicine]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *                 minLength: 1
	 *                 description: Medicine name
	 *                 example: "Updated Email Medicine"
	 *               description:
	 *                 type: string
	 *                 description: Medicine description
	 *                 example: "Updated description for the medicine"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: Medicine type for categorization
	 ...
	 *     description: Permanently delete a medicine by ID
	 *     tags: [Medicine]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
