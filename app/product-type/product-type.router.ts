...
export const router = (route: Router, controller: IController): Router => {
	 ...
	 *     description: Retrieve a specific subCategory by its unique identifier with optional field selection
	 *     tags: [SubCategory]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *         description: SubCategory ID (MongoDB ObjectId format)
	 ...
	 */
	// Cache individual subCategory with predictable key for invalidation
	routes.get(
		"/:id",
		cache({
			ttl: 90,
			keyGenerator: (req: Request) => {
				...
				return `cache:subCategory:byId:${req.params.id}:${fields}`;
			},
		}),
		controller.getById,
	);
	 ...
	 *     description: Retrieve subCategorys with advanced filtering, pagination, sorting, field selection, and optional grouping
	 *     tags: [SubCategory]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *           type: string
	 *           enum: ["true"]
	 *         description: Include subCategory documents in response
	 *       - in: query
	 *         name: pagination
	 *         required: false
	 *         schema:
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	// Cache subCategory list with predictable key for invalidation
	routes.get(
		"/",
		cache({
			ttl: 60,
			keyGenerator: (req: Request) => {
				...
				return `cache:subCategory:list:${queryKey}`;
			},
		}),
		controller.getAll,
	);
	 ...
	 *                 example: "Email Welcome SubCategory"
	 *               description:
	 *                 type: string
	 *                 description: SubCategory description
	 *                 example: "Welcome email subCategory for new users"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: SubCategory type for categorization
	 *                 example: "email"
	 *               isDeleted:
	 *                 type: boolean
	 ...
	 *     description: Update subCategory data by ID (partial update)
	 *     tags: [SubCategory]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *                 minLength: 1
	 *                 description: SubCategory name
	 *                 example: "Updated Email SubCategory"
	 *               description:
	 *                 type: string
	 *                 description: SubCategory description
