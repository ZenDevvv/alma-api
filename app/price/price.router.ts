...
export const router = (route: Router, controller: IController): Router => {
	 ...
	 *     description: Retrieve a specific price by its unique identifier with optional field selection
	 *     tags: [Price]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *         description: Price ID (MongoDB ObjectId format)
	 ...
	 */
	// Cache individual price with predictable key for invalidation
	routes.get(
		"/:id",
		cache({
			ttl: 90,
			keyGenerator: (req: Request) => {
				...
				return `cache:price:byId:${req.params.id}:${fields}`;
			},
		}),
		controller.getById,
	);
	 ...
	 *     description: Retrieve prices with advanced filtering, pagination, sorting, field selection, and optional grouping
	 *     tags: [Price]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *           type: string
	 *           enum: ["true"]
	 *         description: Include price documents in response
	 *       - in: query
	 *         name: pagination
	 *         required: false
	 *         schema:
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	// Cache price list with predictable key for invalidation
	routes.get(
		"/",
		cache({
			ttl: 60,
			keyGenerator: (req: Request) => {
				...
				return `cache:price:list:${queryKey}`;
			},
		}),
		controller.getAll,
	);
	 ...
	 *     description: Create a new price with the provided data
	 *     tags: [Price]
	 *     security:
	 *       - bearerAuth: []
	 *     requestBody:
	 ...
	 *                 example: "Email Welcome Price"
	 *               description:
	 *                 type: string
	 *                 description: Price description
	 *                 example: "Welcome email price for new users"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: Price type for categorization
	 *                 example: "email"
	 *               isDeleted:
	 *                 type: boolean
	 ...
	 *     description: Update price data by ID (partial update)
	 *     tags: [Price]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *                 minLength: 1
	 *                 description: Price name
	 *                 example: "Updated Email Price"
	 *               description:
	 *                 type: string
	 *                 description: Price description
	 *                 example: "Updated description for the price"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: Price type for categorization
	 ...
	 *     description: Permanently delete a price by ID
	 *     tags: [Price]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
