...
export const router = (route: Router, controller: IController): Router => {
	 ...
	 *     description: Retrieve a specific role by its unique identifier with optional field selection
	 *     tags: [Role]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *         description: Role ID (MongoDB ObjectId format)
	 ...
	 */
	// Cache individual role with predictable key for invalidation
	routes.get(
		"/:id",
		cache({
			ttl: 90,
			keyGenerator: (req: Request) => {
				...
				return `cache:role:byId:${req.params.id}:${fields}`;
			},
		}),
		controller.getById,
	);
	 ...
	 *     description: Retrieve roles with advanced filtering, pagination, sorting, field selection, and optional grouping
	 *     tags: [Role]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *           type: string
	 *           enum: ["true"]
	 *         description: Include role documents in response
	 *       - in: query
	 *         name: pagination
	 *         required: false
	 *         schema:
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	// Cache role list with predictable key for invalidation
	routes.get(
		"/",
		cache({
			ttl: 60,
			keyGenerator: (req: Request) => {
				...
				return `cache:role:list:${queryKey}`;
			},
		}),
		controller.getAll,
	);
	 ...
	 *                 example: "Email Welcome Role"
	 *               description:
	 *                 type: string
	 *                 description: Role description
	 *                 example: "Welcome email role for new users"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: Role type for categorization
	 *                 example: "email"
	 *               isDeleted:
	 *                 type: boolean
	 ...
	 *   patch:
	 *     summary: Update role
	 *     description: Update role data by ID (partial update)
	 *     tags: [Role]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
