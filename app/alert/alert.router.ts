...
export const router = (route: Router, controller: IController): Router => {
	 ...
	 *     description: Retrieve a specific alert by its unique identifier with optional field selection
	 *     tags: [Alert]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *         description: Alert ID (MongoDB ObjectId format)
	 ...
	 */
	// Cache individual alert with predictable key for invalidation
	routes.get(
		"/:id",
		cache({
			ttl: 90,
			keyGenerator: (req: Request) => {
				...
				return `cache:alert:byId:${req.params.id}:${fields}`;
			},
		}),
		controller.getById,
	);
	 ...
	 *     description: Retrieve alerts with advanced filtering, pagination, sorting, field selection, and optional grouping
	 *     tags: [Alert]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *           type: string
	 *           enum: ["true"]
	 *         description: Include alert documents in response
	 *       - in: query
	 *         name: pagination
	 *         required: false
	 *         schema:
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	// Cache alert list with predictable key for invalidation
	routes.get(
		"/",
		cache({
			ttl: 60,
			keyGenerator: (req: Request) => {
				...
				return `cache:alert:list:${queryKey}`;
			},
		}),
		controller.getAll,
	);
	 ...
	 *                 example: "Email Welcome Alert"
	 *               description:
	 *                 type: string
	 *                 description: Alert description
	 *                 example: "Welcome email alert for new users"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: Alert type for categorization
	 *                 example: "email"
	 *               isDeleted:
	 *                 type: boolean
	 ...
	 *     description: Update alert data by ID (partial update)
	 *     tags: [Alert]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *                 minLength: 1
	 *                 description: Alert name
	 *                 example: "Updated Email Alert"
	 *               description:
	 *                 type: string
	 *                 description: Alert description
	 *                 example: "Updated description for the alert"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: Alert type for categorization
	 ...
	 *     description: Permanently delete a alert by ID
	 *     tags: [Alert]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
