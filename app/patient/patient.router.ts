...
export const router = (route: Router, controller: IController): Router => {
	 ...
	 *     description: Retrieve a specific patient by its unique identifier with optional field selection
	 *     tags: [Patient]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *         description: Patient ID (MongoDB ObjectId format)
	 ...
	 */
	// Cache individual patient with predictable key for invalidation
	routes.get(
		"/:id",
		cache({
			ttl: 90,
			keyGenerator: (req: Request) => {
				...
				return `cache:patient:byId:${req.params.id}:${fields}`;
			},
		}),
		controller.getById,
	);
	 ...
	 *     description: Retrieve patients with advanced filtering, pagination, sorting, field selection, and optional grouping
	 *     tags: [Patient]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *           type: string
	 *           enum: ["true"]
	 *         description: Include patient documents in response
	 *       - in: query
	 *         name: pagination
	 *         required: false
	 *         schema:
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	// Cache patient list with predictable key for invalidation
	routes.get(
		"/",
		cache({
			ttl: 60,
			keyGenerator: (req: Request) => {
				...
				return `cache:patient:list:${queryKey}`;
			},
		}),
		controller.getAll,
	);
	 ...
	 *                 example: "Email Welcome Patient"
	 *               description:
	 *                 type: string
	 *                 description: Patient description
	 *                 example: "Welcome email patient for new users"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: Patient type for categorization
	 *                 example: "email"
	 *               isDeleted:
	 *                 type: boolean
	 ...
	 *     description: Update patient data by ID (partial update)
	 *     tags: [Patient]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *                 minLength: 1
	 *                 description: Patient name
	 *                 example: "Updated Email Patient"
	 *               description:
	 *                 type: string
	 *                 description: Patient description
	 *                 example: "Updated description for the patient"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: Patient type for categorization
