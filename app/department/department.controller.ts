...
const locationLogger = logger.child({ module: "department" });
...
export const controller = (prisma: PrismaClient) => {
	const create = async (req: AuthRequest, res: Response, _next: NextFunction) => {
		...
		try {
			const location = await prisma.department.create({
				data: {
					...validation.data,
				},
			});
	...
	const getAll = async (req: AuthRequest, res: Response, _next: NextFunction) => {
		...
		try {
			...
			const whereClause: Prisma.DepartmentWhereInput = {
				isDeleted: false,
				...(req.orgId && req.departmentCode !== "PPPOF" && { orgId: req.orgId }),
			};

			// search fields sample ("name", "description", "type")
			const searchFields = ["name", "code", "contactPerson", "contactPhone", "contactEmail"];
			if (query) {
			...
			locationLogger.info(`Retrieved ${locations.length} Department`);
			const processedData =
		...
		try {
			...
			if (!department) {
				...
				if (department && redisClient.isClientConnected()) {
					try {
						await redisClient.setJSON(cacheKey, department, 3600); // ✅ Fixed
						locationLogger.info(`Department ${id} stored in direct Redis cache`);
					} catch (cacheError) {
						locationLogger.warn(
			...
			locationLogger.info(`${config.SUCCESS.LOCATION.RETRIEVED}: ${(department as any).id}`);
			const successResponse = buildSuccessResponse(
	...
	const update = async (req: Request, res: Response, _next: NextFunction) => {
		...
		try {
			...
			const existingLocation = await prisma.department.findFirst({
				where: { id },
			});
			...
			const updatedLocation = await prisma.department.update({
				where: { id },
