...
interface IController {
	...
	approvedUpdate(req: Request, res: Response, next: NextFunction): Promise<void>;
	remove(req: Request, res: Response, next: NextFunction): Promise<void>;
}

export const router = (route: Router, controller: IController): Router => {
	const routes = Router();
	const path = "/delivery-request";
	 ...
	 *     description: Retrieve a specific delivery by its unique identifier with optional field selection
	 *     tags: [Delivery]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *         description: Delivery ID (MongoDB ObjectId format)
	 ...
	 */
	// Cache individual delivery with predictable key for invalidation
	routes.get(
		"/:id",
		cache({
			ttl: 90,
			keyGenerator: (req: Request) => {
				...
				return `cache:delivery:byId:${req.params.id}:${fields}`;
			},
		}),
		controller.getById,
	);
	 ...
	 *     description: Retrieve deliverys with advanced filtering, pagination, sorting, field selection, and optional grouping
	 *     tags: [Delivery]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *           type: string
	 *           enum: ["true"]
	 *         description: Include delivery documents in response
	 *       - in: query
	 *         name: pagination
	 *         required: false
	 *         schema:
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	// Cache delivery list with predictable key for invalidation
	routes.get(
		"/",
		cache({
			ttl: 60,
			keyGenerator: (req: Request) => {
				...
				return `cache:delivery:list:${queryKey}`;
			},
		}),
		controller.getAll,
	);
	 ...
	 *     description: Create a new delivery with the provided data
	 *     tags: [Delivery]
	 *     security:
	 *       - bearerAuth: []
	 *     requestBody:
	 ...
	 *                 example: "Email Welcome Delivery"
	 *               description:
	 *                 type: string
	 *                 description: Delivery description
	 *                 example: "Welcome email delivery for new users"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: Delivery type for categorization
	 *                 example: "email"
	 *               isDeleted:
	 *                 type: boolean
	 ...
	 *     description: Update delivery data by ID (partial update)
	 *     tags: [Delivery]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *                 minLength: 1
	 *                 description: Delivery name
	 *                 example: "Updated Email Delivery"
	 *               description:
	 *                 type: string
	 *                 description: Delivery description
	 *                 example: "Updated description for the delivery"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: Delivery type for categorization
	...
	routes.patch("/:id/bulk", controller.approvedUpdate);
	 ...
	 *     description: Permanently delete a delivery by ID
	 *     tags: [Delivery]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
