...
import { checkApprovalAssignment } from "../../middleware/check-approval-assign";
...
export const router = (route: Router, controller: IController): Router => {
	 ...
	 *     description: Retrieve a specific approval by its unique identifier with optional field selection
	 *     tags: [Approval]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *         description: Approval ID (MongoDB ObjectId format)
	 ...
	 */
	// Cache individual approval with predictable key for invalidation
	routes.get(
		"/:id",
		cache({
			ttl: 90,
			keyGenerator: (req: Request) => {
				...
				return `cache:approval:byId:${req.params.id}:${fields}`;
			},
		}),
		controller.getById,
	);
	 ...
	 *     description: Retrieve approvals with advanced filtering, pagination, sorting, field selection, and optional grouping
	 *     tags: [Approval]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *           type: string
	 *           enum: ["true"]
	 *         description: Include approval documents in response
	 *       - in: query
	 *         name: pagination
	 *         required: false
	 *         schema:
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	// Cache approval list with predictable key for invalidation
	routes.get(
		"/",
		cache({
			ttl: 60,
			keyGenerator: (req: Request) => {
				...
				return `cache:approval:list:${queryKey}`;
			},
		}),
		controller.getAll,
	);
	 ...
	 *     description: Create a new approval with the provided data
	 *     tags: [Approval]
	 *     security:
	 *       - bearerAuth: []
	 *     requestBody:
	 ...
	 *                 example: "Email Welcome Approval"
	 *               description:
	 *                 type: string
	 *                 description: Approval description
	 *                 example: "Welcome email approval for new users"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: Approval type for categorization
	 *                 example: "email"
	 *               isDeleted:
	 *                 type: boolean
	 ...
	 *     description: Update approval data by ID (partial update)
	 *     tags: [Approval]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *                 minLength: 1
	 *                 description: Approval name
	 *                 example: "Updated Email Approval"
	 *               description:
	 *                 type: string
	 *                 description: Approval description
	 *                 example: "Updated description for the approval"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: Approval type for categorization
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	routes.patch("/:id", checkApprovalAssignment, controller.update);
	 ...
	 *     description: Permanently delete a approval by ID
	 *     tags: [Approval]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
