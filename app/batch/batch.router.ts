...
export const router = (route: Router, controller: IController): Router => {
	 ...
	 *     description: Retrieve a specific batch by its unique identifier with optional field selection
	 *     tags: [Batch]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *         description: Batch ID (MongoDB ObjectId format)
	 ...
	 */
	// Cache individual batch with predictable key for invalidation
	routes.get(
		"/:id",
		cache({
			ttl: 90,
			keyGenerator: (req: Request) => {
				...
				return `cache:batch:byId:${req.params.id}:${fields}`;
			},
		}),
		controller.getById,
	);
	 ...
	 * /api/batch:
	 *   get:
	 *     summary: Get all batchs
	 *     description: Retrieve batchs with advanced filtering, pagination, sorting, field selection, and optional grouping
	 *     tags: [Batch]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *           type: string
	 *           enum: ["true"]
	 *         description: Include batch documents in response
	 *       - in: query
	 *         name: pagination
	 *         required: false
	 *         schema:
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	// Cache batch list with predictable key for invalidation
	routes.get(
		"/",
		cache({
			ttl: 60,
			keyGenerator: (req: Request) => {
				...
				return `cache:batch:list:${queryKey}`;
			},
		}),
		controller.getAll,
	);
	 ...
	 *     description: Create a new batch with the provided data
	 *     tags: [Batch]
	 *     security:
	 *       - bearerAuth: []
	 *     requestBody:
	 ...
	 *                 example: "Email Welcome Batch"
	 *               description:
	 *                 type: string
	 *                 description: Batch description
	 *                 example: "Welcome email batch for new users"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: Batch type for categorization
	 *                 example: "email"
	 *               isDeleted:
	 *                 type: boolean
	 ...
	 *     description: Update batch data by ID (partial update)
	 *     tags: [Batch]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *     description: Permanently delete a batch by ID
	 *     tags: [Batch]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
