...
export const router = (route: Router, controller: IController): Router => {
	 ...
	 *     description: Retrieve a specific organization by its unique identifier with optional field selection
	 *     tags: [Organization]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *         description: Organization ID (MongoDB ObjectId format)
	 ...
	 */
	// Cache individual organization with predictable key for invalidation
	routes.get(
		"/:id",
		cache({
			ttl: 90,
			keyGenerator: (req: Request) => {
				...
				return `cache:organization:byId:${req.params.id}:${fields}`;
			},
		}),
		controller.getById,
	);
	 ...
	 *     description: Retrieve organizations with advanced filtering, pagination, sorting, field selection, and optional grouping
	 *     tags: [Organization]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *           type: string
	 *           enum: ["true"]
	 *         description: Include organization documents in response
	 *       - in: query
	 *         name: pagination
	 *         required: false
	 *         schema:
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	// Cache organization list with predictable key for invalidation
	routes.get(
		"/",
		cache({
			ttl: 60,
			keyGenerator: (req: Request) => {
				...
				return `cache:organization:list:${queryKey}`;
			},
		}),
		controller.getAll,
	);
	 ...
	 *                 example: "Email Welcome Organization"
	 *               description:
	 *                 type: string
	 *                 description: Organization description
	 *                 example: "Welcome email organization for new users"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: Organization type for categorization
	 *                 example: "email"
	 *               isDeleted:
	 *                 type: boolean
	 ...
	 *     description: Update organization data by ID (partial update)
	 *     tags: [Organization]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *                 minLength: 1
	 *                 description: Organization name
	 *                 example: "Updated Email Organization"
	 *               description:
	 *                 type: string
	 *                 description: Organization description
	 ...
	 *     description: Permanently delete a organization by ID
	 *     tags: [Organization]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
