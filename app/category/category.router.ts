...
export const router = (route: Router, controller: IController): Router => {
	 ...
	 */
	// Cache individual category with predictable key for invalidation
	routes.get(
		"/:id",
		cache({
			ttl: 90,
	 ...
	 *     description: Retrieve categorys with advanced filtering, pagination, sorting, field selection, and optional grouping
	 *     tags: [Category]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	// Cache category list with predictable key for invalidation
	routes.get(
		"/",
		cache({
			ttl: 60,
