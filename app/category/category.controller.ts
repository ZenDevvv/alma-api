...
export const controller = (prisma: PrismaClient) => {
	const create = async (req: Request, res: Response, _next: NextFunction) => {
		...
		try {
			...
			logActivity(req, {
				...
				action: config.ACTIVITY_LOG.CATEGORY.ACTIONS.CREATE,
				description: `${config.ACTIVITY_LOG.CATEGORY.DESCRIPTIONS.CREATED}: ${category.name || category.id}`,
				page: {
					url: req.originalUrl,
					title: config.ACTIVITY_LOG.CATEGORY.PAGES.CREATION,
				},
			});
			...
			logAudit(req, {
				...
				description: `${config.AUDIT_LOG.CATEGORY.DESCRIPTIONS.CREATED}: ${category.name || category.id}`,
			});
	...
	const getAll = async (req: Request, res: Response, _next: NextFunction) => {
		...
		categoryLogger.info(
			`Getting categories, page: ${page}, limit: ${limit}, query: ${query}, order: ${order}, groupBy: ${groupBy}`,
		);
		...
		try {
			...
			const searchFields = ["name", "description", "status"];
			if (query) {
			...
			const processedData =
				groupBy && document ? groupDataByField(categories, groupBy as string) : categories;
